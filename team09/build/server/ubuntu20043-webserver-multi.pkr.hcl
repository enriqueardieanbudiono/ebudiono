...
#################################################################
# Build for Ubuntu Focal 20.04 Webserver template
# https://www.packer.io/docs/builders/proxmox/iso
#################################################################
...
build {
  sources = ["source.virtualbox-iso.ws1",
  "source.virtualbox-iso.ws2",
  "source.virtualbox-iso.ws3",
  "source.virtualbox-iso.lb",
  "source.virtualbox-iso.db",
  ...
  provisioner "file" {
    ...
    only = ["proxmox-iso.ws", "proxmox-iso.lb", "proxmox-iso.db"]
  }
   ########################################################################################################################
...
#########################################################################################
  ...
  ########################################################################################################################
  # Script for setting up the virtualbox-iso.ws
  ########################################################################################################################
  ...
  provisioner "shell" {
    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
    environment_vars = ["NUMBER=${var.TEAMNUMBER}"]
    script          = "../scripts/core-focal/post_install_ubuntu_lb_proxmox.sh"
    only            = ["proxmox-iso.lb"]
  }
  ...
  provisioner "shell" {
    execute_command = "echo 'vagrant' | {{ .Vars }} sudo -E -S sh '{{ .Path }}'"
    environment_vars = ["NUMBER=${var.TEAMNUMBER}"]
    script          = "../scripts/core-focal/post_install_ubuntu_ws_proxmox.sh"
    only            = ["proxmox-iso.ws"]
  }
  ...
  provisioner "shell" {
    ...
    environment_vars = ["USERPASS=${var.non-root-user-for-database-password}",
                        ...
                        "USERNAME=${var.non-root-user-for-database-username}",
                        "NUMBER=${var.TEAMNUMBER}"]
    script          = "../scripts/core-focal/post_install_ubuntu_db_proxmox.sh"
    only            = ["proxmox-iso.db"]
  }
