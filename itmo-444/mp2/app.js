...
//	console.log("Topic Arn: ",snsTopicArn.Topics[0].TopicArn);
	const params = {
...
const subscribeEmailToSNSTopic = async (req,res) => {
        ...
        console.log("Topic ARN: ", topicArn.Topics[0].TopicArn);
...
///////////////////////////////////////////////
// Special for Image Gallery
///////////////////////////////////////////////
const listObjectforGallery = async () => {
        const client = new S3Client({region: "us-east-1" });
        const command = new ListObjectsCommand(await listBuckets());
        try {
                const results = await client.send(command);
                ...
                // Loop through the results and create an array of URLs
                var urls = [];
                for (var i = 0; i < results.Contents.length; i++) {
                        var url = "https://" + results.Name + ".s3.amazonaws.com/" + results.Contents[i].Key;
                        urls.push(url);
                }
                console.log(urls);
                return urls;
...
const getImagesFromS3Bucket = async (req,res) => {
	try {
		let imageURL = await listObjectforGallery();

                console.log("Image in bucket: ", imageURL.length);
                console.log("Image URL: ", imageURL);
                res.set('Content-Type', 'text/html');	
                res.write("Welcome to the gallery" + "\n");
                // Loop through the array of URLs and write them to the screen
